...
namespace margelo::nitro::test::views {
  ...
  /**
   * C++ <-> Kotlin bridge for the "TestView" measurer. All `jclass`/method
   * lookups are cached; the per-measure path is one cached static call.
   */
  struct JHybridTestViewMeasurer final: public jni::JavaClass<JHybridTestViewMeasurer> {
    static constexpr auto kJavaDescriptor = "Lcom/margelo/nitro/test/views/HybridTestViewMeasurer;";
    ...
    static void install() {
      ...
      HybridTestViewShadowNode::measureFunction.store(
        [](const react::ViewProps& propsBase,
          ...
          const auto& props = static_cast<const HybridTestViewProps&>(propsBase);
          margelo::nitro::test::TestViewProps measureProps = margelo::nitro::test::TestViewProps { props.isBlue.value, props.hasBeenCalled.value, props.colorScheme.value };
          ...
          static const auto measure = javaClassStatic()
            ->getStaticMethod<jlong(jni::alias_ref<JTestViewProps::javaobject>, jdouble, jboolean, jdouble, jdouble, jdouble, jdouble)>("measure");
          jlong packed = measure(
            javaClassStatic(),
            JTestViewProps::fromCpp(measureProps),
