 ...
 * 2. To avoid exposing the whole inheritance chain to C++, we only expose the methods HybridSomeExternalObjectSubclassSpec defines itself,
 *    otherwise C++ would see HybridSomeExternalObjectSubclassSpec's base class and if that's an external symbol the build fails.
 ...
 */
public final class HybridSomeExternalObjectSubclassSpec_cxx {
  /**
  ...
  @inline(__always)
  private static func cast(_ this: UnsafeRawPointer) -> HybridSomeExternalObjectSubclassSpec {
  ...
  // Methods
  @inline(__always)
  public static func getSubclassedValue(this: UnsafeRawPointer) -> bridge.Result_std__string_ {
    do {
      let __instance = cast(this)
      let __result = try __instance.getSubclassedValue()
