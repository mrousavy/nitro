...
open class HybridTestObjectSwiftKotlinSpec_cxx {
  ...
  @inline(__always)
  public final func bounceNestedStructs(array: bridge.std__vector_Car_) -> bridge.Result_std__vector_Car__ {
    do {
      let __result = try self.__implementation.bounceNestedStructs(array: { () -> [Car] in
        let __data = bridge.get_data_std__vector_Car_(array)
        let __size = array.size()
        return Array(UnsafeBufferPointer(start: __data, count: __size))
      }())
      let __resultCpp = __result.withUnsafeBufferPointer { __pointer -> bridge.std__vector_Car_ in
        return bridge.copy_std__vector_Car_(__pointer.baseAddress!, __result.count)
      }
      return bridge.create_Result_std__vector_Car__(__resultCpp)
    } catch (let __error) {
      let __exceptionPtr = __error.toCpp()
      return bridge.create_Result_std__vector_Car__(__exceptionPtr)
    }
  }
