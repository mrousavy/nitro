...
public:
  ...
  Promise<int64_t> calculateFibonacciAsync(double value) override;
  Promise<void> wait(double seconds) override;
  ...
  Promise<double> getValueFromJSCallbackAndWait(const std::function<Promise<double>()>& getValue) override;
  void callAll(const std::function<void()>& first, const std::function<void()>& second, const std::function<void()>& third) override;
  Promise<void> getValueFromJsCallback(const std::function<Promise<std::string>()>& callback,
                                       const std::function<void(const std::string& /* valueFromJs */)>& andThenCall) override;
  Promise<void> promiseThrows() override;
  Car getCar() override;
  ...
  Promise<std::shared_ptr<ArrayBuffer>> createArrayBufferAsync() override;
