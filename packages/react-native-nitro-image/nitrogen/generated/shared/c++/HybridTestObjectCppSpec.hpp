...
namespace margelo::nitro::image {
  ...
  class HybridTestObjectCppSpec: public virtual HybridObject {
    ...
    public:
      ...
      virtual std::shared_ptr<Promise<double>> getValueFromJSCallbackAndWait(const std::shared_ptr<Callback<double()>>& getValue) = 0;
      virtual std::shared_ptr<Promise<void>> getValueFromJsCallback(const std::shared_ptr<Callback<std::string()>>& callback, const std::shared_ptr<Callback<void(const std::string& /* valueFromJs */)>>& andThenCall) = 0;
      ...
      virtual void complexEnumCallback(const std::vector<Powertrain>& array, const std::shared_ptr<Callback<void(const std::vector<Powertrain>& /* array */)>>& callback) = 0;
      ...
      virtual void callCallback(const std::shared_ptr<Callback<void()>>& callback) = 0;
      virtual void callAll(const std::shared_ptr<Callback<void()>>& first, const std::shared_ptr<Callback<void()>>& second, const std::shared_ptr<Callback<void()>>& third) = 0;
      virtual void callWithOptional(std::optional<double> value, const std::shared_ptr<Callback<void(std::optional<double> /* maybe */)>>& callback) = 0;
