...
export function createCMakeExtension(files: SourceFile[]): CMakeFile {
  ...
  const hasHybridViews = files.some(
    (file) =>
      file.platform === 'android' && file.name.endsWith('StateUpdater.cpp')
  )
  const compileDefinitions = [
    buildingWithDefinition,
    ...(hasHybridViews ? ['RN_SERIALIZABLE_STATE'] : []),
  ]
...
target_compile_definitions(
  ${name} PRIVATE
  ${indent(compileDefinitions.join('\n'), '  ')}
)
