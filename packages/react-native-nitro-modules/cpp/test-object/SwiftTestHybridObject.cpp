...
std::shared_ptr<SwiftTestHybridObject> SwiftTestHybridObject::getHybridPart(NitroModules::SwiftTestHybridObjectSwift swiftPart) {
  ...
  swift::Optional<HybridContext> hybridContext = swiftPart.getHybridContext();
  if (hybridContext.isSome()) {
    // 1.1. Get Weak reference to the C++ HybridObject
    std::weak_ptr<HybridObject> weakHybridObject = hybridContext.get().cppPart;
    std::shared_ptr<HybridObject> hybridObject = weakHybridObject.lock();
